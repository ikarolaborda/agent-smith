...
func TestCapabilityStatusIsTruthfulByDefault(t *testing.T) {
	...
	for _, name := range []string{"host_shell", "arbitrary_http", "contained_execution", "coverage_guided_fuzzing", "artifact_persistence", "research_persistence", "authentication", "research_query_tool", "research_artifact_encryption", "research_artifact_retention", "research_supply_chain_admission", "research_cpu_rate_limit", "research_writable_monitor", "research_kernel_storage_quota", "research_full_accounting"} {
